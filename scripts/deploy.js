...
const UPLOAD_RELEASE_URL =
    `https://uploads.github.com/repos/${REPO}/releases/{id}/assets?access_token=${ACCESS_TOKEN}&name={name}`;

const RELEASE_BODY =
    `Released at {date}\r\n\r\nRelease for commit: https://github.com/uEssentials/uEssentials/commit/${COMMIT} (${
...
function uploadAsset(releaseInfo, filePath, fileName) {
    ...
    let res = request('POST',
        UPLOAD_RELEASE_URL.replace('{id}', id).replace('{name}', fileName),
        {
            headers: {
                'User-Agent': 'node',
...
let fileInfoResp = request('GET',
    `https://api.github.com/repos/${REPO}/contents/README.md?access_token=${ACCESS_TOKEN}`,
    {
...
request("POST",
    `https://api.github.com/repos/uessentials/uessentials/statuses/${COMMIT}?access_token=${ACCESS_TOKEN}`,
    {
        headers: { 'User-Agent': 'node' },
        body: JSON.stringify({
            state: "success",
